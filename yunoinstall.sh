...
$bold
  ...
  â”‚ YunoHost Installation â”‚
  â•°â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â•¯
$normal
  â€¢ Installing YunoHost requires to install various important services,
...
EOF
  ...
  â€¢ You are installing the unstable/alpha version of YunoHost 12/Bookworm.
...
EOF
    ...
    then
        ...
        if ! grep -E "^[[:blank:]]*PermitRootLogin[[:blank:]]+no" /etc/ssh/sshd_config ; then
            root_login_warning="      â€¢ SSH login using root will be disabled (except from local network).\n"
        ...
        if grep -Ev "^[[:blank:]]*Port[[:blank:]]+22[[:blank:]]*(#.*)?$" /etc/ssh/sshd_config | grep -E "^[[:blank:]]*Port[[:blank:]]+[[:digit:]]+$" ; then
            ssh_port_warning="      â€¢ You will have to connect using port 22 instead of your custom SSH port,\n"
  ...
  â€¢ Additionally, it is encouraged to let YunoHost manage the SSH configuration.
...
EOF
  ...
  ğŸš€ ${bold}Let's go !$normal

  ğŸ“œ Detailed logs will be available in $YUNOHOST_LOG
EOF
...
function upgrade_system() {
    ...
    echo "$bold  1/5 â€¢ Running system upgrades$normal" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
...
function boring_workarounds() {
    ...
    echo "" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
    echo "$bold  2/5 â€¢ Install dependencies needed before the main install$normal" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
    ...
    echo "" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
    echo "$bold  3/5 â€¢ Apply various tweaks to prepare installation$normal" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
...
function setup_package_source() {
    ...
    echo "" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
    echo "$bold  4/5 â€¢ Adding YunoHost repository to apt$normal" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
...
function install_yunohost_packages() {
    ...
    echo "" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
    echo "$bold  5/5 â€¢ Installing YunoHost$normal" | tee -a "$YUNOHOST_LOG"
    echo "" | tee -a "$YUNOHOST_LOG"
...
function conclusion() {
    ...
    local width=79
    [[ -z "$local_ip" ]] || {
        local_ip=$(echo -e "\n  â”‚    - https://$local_ip/ (local IP, if self-hosting at home)")
        local nb_spaces=$(( width - ${#local_ip} ))
        local_ip+="$(printf "%${nb_spaces}s")â”‚"
    }
    [[ -z "$global_ip" ]] || {
        global_ip=$(echo -e "\n  â”‚    - https://$global_ip/ (global IP, if you're on a VPS)")
        local nb_spaces=$(( width - ${#global_ip} ))
        global_ip+="$(printf "%${nb_spaces}s")â”‚"
    }
    [[ -n "$local_ip" ]] || [[ -n "$global_ip" ]] || {
        no_ip=$(echo -e "\n  â”‚    - (no local nor global IP detected ?)")
        local nb_spaces=$(( width - ${#no_ip} ))
        no_ip+="$(printf "%${nb_spaces}s")â”‚"
    }

    cat << EOF | tee -a "$YUNOHOST_LOG"


  ğŸ‰ ${bold}YunoHost installation completed!$normal

  â•­â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â•®
  â”‚ You should now proceed with Yunohost post-installation.                   â”‚
  â”‚ This is where you will be asked for:                                      â”‚
  â”‚ â€¢ the main domain of your server ;                                        â”‚
  â”‚ â€¢ the administration password ;                                           â”‚
  â”‚ â€¢ the name and password of the first user, which will also be admin.      â”‚
  â”‚                                                                           â”‚
  â”‚ You can perform this step, either:                                        â”‚
  â”‚ â€¢ from the command line, by running 'yunohost tools postinstall' as root  â”‚
  â”‚ â€¢ or from your web browser, by accessing :                                â”‚${local_ip}${global_ip}${no_ip}
  â”‚                                                                           â”‚
  â”‚ If this is your first time with YunoHost, it is strongly recommended to   â”‚
  â”‚ take time to read the administator documentation and in particular the    â”‚
  â”‚ sections 'Finalizing your setup' and 'Getting to know YunoHost'.          â”‚
  â”‚                                                                           â”‚
  â”‚ It is available at the following URL : â¡ï¸  https://yunohost.org/admindoc   â”‚
  â•°â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â•¯
